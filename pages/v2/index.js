...
export default function Home() {
  ...
  const _fetchMarketItems = async () => {
    ...
    const data1 = await marketContract.fetchItemsListed();
    const newListed = await Promise.all(data1.map(async i => {
          ...
          <div className="row">
            {
              ...
              items.map((item, index) => 
                <div className="col-md-6 col-lg-3 mb-4" key={index}>
                  <Link href={`/details/${item.nftContract}/${item.tokenId}`}>
                    <div className="card-item">
