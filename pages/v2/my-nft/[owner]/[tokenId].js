...
export default function MyNFT({owner, tokenId}) {
	...
	const _createMarketSale = async (item) => {
	  ...
	  const _provider = window.ethereum ? new ethers.providers.Web3Provider(window.ethereum) : ethers.providers.getDefaultProvider();
	  const marketContract = new ethers.Contract(marketContractAddress.contractAddress, Market.abi, _provider.getSigner(0));
	  ...
	  const transaction = await marketContract.createMarketSale(nftContractAddress.contractAddress, item.tokenId, {
	    value: price
	  });
						...
						<div className="row">
							...
							<div className="col-lg-6">
								...
								<div className="text-white fw-bolder mb-4">
									<span>Owned by </span>
									{
										item.owner.substring(0, 4) == '0x00'?
										<span className="text-gradient">
											{item.seller?.substring(0, 4)}...{item.seller?.substring(item.seller.length - 4, item.seller.length)}
										</span> :
										<span className="text-gradient">
								...
								<div className="card-collapse mb-4">
									...
									<div className="card-collapse-content">
										<div className="d-flex justify-content-between">
											...
											<span className="fw-bold text-gradient">{item.nftContract?.substring(0, 4)}...{item.nftContract?.substring(item.nftContract.length - 4, item.nftContract.length)}</span>
										</div>
										<div className="d-flex justify-content-between">
											<span>Token ID</span>
								...
								<div className="card bg-blur border-0">
									<div className="card-body">
										...
										<h2 className="text-white">{item.price} ETH</h2>
										<button 
											type="button" 
